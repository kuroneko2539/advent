...
parser.add_argument("--inFile", type=str, default="./day25input.txt", help="file containing input key")
...
while True:
    ...
    if instruction == "cpy":
        ...
        outCount += 1
        last20.append(registers0["b"])
        if outCount == 20:
            for reg in [x for x in "abcd"]:
                registers0[reg] = 0
            registers0["a"] = currentA + 1
            currentA += 1
            pos0 = -1
            outCount = 0
            if "".join([str(x) for x in last20]) == "01010101010101010101": 
                print(currentA - 1)
                break
            last20 = []
